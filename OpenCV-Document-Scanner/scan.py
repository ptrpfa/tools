...
def read_heic(image_path):
    ...
    new_path = image_path.replace("heic", "png")
    new_path = image_path.replace("HEIC", "png")
...
class DocScanner(object):
    ...
    def scan(self, image_path):
        ...
        if image_path.endswith('.HEIC') or image_path.endswith('.heic'):
            image_path = read_heic(image_path)
        ...
        rotate = cv2.rotate(thresh, cv2.ROTATE_90_CLOCKWISE)

        # save the transformed image
        basename = os.path.basename(image_path)
        cv2.imwrite(OUTPUT_DIR + '/' + basename, rotate)
...
if __name__ == "__main__":
    ...
    valid_formats = [".jpg", ".jpeg", ".jp2", ".png", ".bmp", ".tiff", ".tif", ".heic"]
